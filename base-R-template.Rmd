...
```{r}
subset_data<-subset(iris,Petal.Width>1)
average_speal_length<-mean(subset_data$Sepal.Length)
...
plotNormal <- function(repitions, mean, sd) {
  random_nums <- rnorm(repitions, mean = mean, sd = sd)
  hist(random_nums, main = "Histogram of Random Normal Numbers",
...
plotNormal <- function(repitions, mean, sd, col = "lightblue") {
  random_nums <- rnorm(repitions, mean = mean, sd = sd)
  hist(random_nums, main = paste0("Histogram from a Normal(", mean, ", ", sd, ") Distribution"),
       xlab = "Value", ylab = "Frequency", col = col)
}
plotNormal(1000, mean = 0, sd = 1, col = "blue")
